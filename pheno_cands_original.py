...
patos = parse_pato(PATO_ONTOLOGY) + ['alter', 'growth', 'develop', 'affect', 'display', 'twice']
...
LINKS = DictionaryMatch(d=['IN', 'DT', 'TO', 'CC', 'WDT', 'PRP'], longest_match_only=True, attrib='pos_tags')#RegexMatchSpan(rgx=r'^(IN|TO|DT|CC)$', longest_match_only=True, attrib='pos_tags')
...
PREPS = DictionaryMatch(d=['IN', 'DT', 'TO', 'PRP'], longest_match_only=True, attrib='pos_tags')
...
PATO = Sequence(Union(PATO, ADJS, NN_ADJ, longest_match_only = True), NUMS, ADVBS, NOS, COMPS, required=[1,0,0,0,0], punct='punct', links = [LINKS, LINK_VBS], longest_match_only=True)
#PATO = Concat(PATO, NN_PHRASE, permutations=True, right_required=False, longest_match_only=True)
