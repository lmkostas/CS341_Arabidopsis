...
*Note - this script returns only the 'Results' section of each article
    ...
    with open(tsv_file, 'w') as wf:
        ...
        with open(raw_xml_file, 'rb') as f:
            ...
            for i, doc in enumerate(html.find_all('article')):
                ...
                text = ''
                secRes = doc.find(attrs={"sec-type": "results"})
                isSecType = False
                ...
                if doc.find('body') and (isSecType or isTitleType):
                    small_file_count += 1
                    if idISSelected:
                        req_file_count += 1
                    else:
                    ...
                    #See above cases statements -> # text = replace_all_escaped(''.join(doc.find('body').find_all(text=True)))
